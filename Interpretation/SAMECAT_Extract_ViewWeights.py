...
def samecat_validation_results(tune, miostone_tree, loaded_data_test, 
    ...
    for _, row in tune.iterrows():
        ...
        raw_weights = model.fusion.weights.detach().cpu()
        normalized_weights = torch.softmax(raw_weights, dim=0)
        ...
        out = {
            ...
            **samecat_params,
            "Raw mgs weight": raw_weights[0].detach().cpu().numpy(),
            "Raw clinical weight": raw_weights[1].detach().cpu().numpy(),
            "Normalized mgs weight": normalized_weights[0].detach().cpu().numpy(),
            "Normalized clinical weight": normalized_weights[1].detach().cpu().numpy()
        }
