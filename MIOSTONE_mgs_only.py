...
class MIOSTONE_mgs_only(nn.Module):
    def __init__(self, 
                 tree,
                 mra_out_dim,
                 node_min_dim,
                 ...
                 prune_mode,  
                 dc_h_dim):
        super(MIOSTONE_mgs_only, self).__init__()
        ...
        self.decoder = nn.Sequential(
            nn.Linear(mra_out_dim, dc_h_dim),
            #nn.BatchNorm1d(dc_h_dim),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Linear(dc_h_dim, 1)
        )
        
    def forward(self, mgs_data):
        mgs_code = self.mra_encoder(mgs_data)
        pred_htot = self.decoder(mgs_code)
...
class Objective:
    def __init__(self, tree_path, master_path, div, bmd_site, dtype = torch.float64):
    ...
    def __call__(self, trial):
        ...
        model = MIOSTONE_mgs_only(tree = miostone_tree,
        ...
        for epoch2 in range(1, p2_epoch_num + 1):
            ...
            pred_htot, total_l0_reg = model(loaded_data_train.X)
            pred_loss = mse_loss(pred_htot, loaded_data_train.y)
            ...
            model.eval()
            with torch.no_grad():
                pred_htot_valid, _ = model(loaded_data_valid.X)
...
def testMIOSTONE_mgs_only(tree_path, master_path, div, bmd_site, 
    ...
    for epoch2 in range(1, p2_epoch_num + 1):
        ...
        pred_htot, total_l0_reg = model(loaded_data_tune.X)
        pred_loss = mse_loss(pred_htot, loaded_data_tune.y)
        loss = pred_loss + lambda_2*total_l0_reg
        loss_tune = loss.item()
        ...
        model.eval()
        with torch.no_grad():
            pred_htot_test, _ = model(loaded_data_test.X)
    ...
    torch.save(model.state_dict(), model_path + div + '_' + bmd_site + '_miostone_mgs_only_testing.pt')
    ...
    rmse_r2_dict = {}
    tune_rmse_dict = {'Tuning RMSE': np.sqrt(np.array(pred_loss.detach().cpu().numpy()))}
    ...
    if save_pred_results:
        ...
        tune_pred_cache.to_csv(pred_save_path + div + '_' + bmd_site + '_tune_set_miostone_mgs_only_pred_results.csv', index=False)
        test_pred_cache.to_csv(pred_save_path + div + '_' + bmd_site + '_test_set_miostone_mgs_only_pred_results.csv', index=False)
...
model_path = root_path + 'saved_models_miostone_mgs_only/'
os.makedirs(model_path, exist_ok=True)
init_model_params_dict = {'mra_out_dim', 'dc_h_dim'}
summarized_results_path = root_path + 'summarized_results_miostone_mgs_only/'
...
for div in div_list:
    ...
    objective = Objective(tree_path, master_path, div, bmd_site)
    study.optimize(objective, n_trials=100)
    
    best_params_dict = study.best_trial.params
    summarized_results_dict = testMIOSTONE_mgs_only(tree_path, master_path, div, bmd_site, 
                                                    best_params_dict, init_model_params_dict, model_path)
...
summarized_results_cache.to_csv(summarized_results_path + bmd_site + '_miostone_mgs_only_summarized_result.csv', index=False)
