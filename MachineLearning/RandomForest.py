...
def optimize_random_forest(X_train, mra_train, y_train,
                           X_valid, mra_valid, y_valid,
                           X_tune, mra_tune, y_tune,
                           X_test, mra_test, y_test,
                           n_trials = 100, conduct_clr_transformation = True):
    """
    Optimize Random Forest hyperparameters using Optuna, train on the combined tuning set,
    ...
    def objective(trial):
        ...
        param = {
            ...
            'max_features': trial.suggest_categorical('max_features', ['sqrt', 'log2']) # Changed 'auto' to 'sqrt'
        }

        # Initialize the Random Forest Regressor with current hyperparameters
        ...
        # Calculate RMSE
        rmse_valid = mean_squared_error(y_valid, y_valid_pred) ** 0.5
        return rmse_valid
    ...
    for PB in PB_methods:
        ...
        for div in div_list:
            ...
            subject_id_valid, mra_valid, X_valid, Y_valid = load_data(data_path + div + "/",
                                                                      ...
                                                                      only_clinicvar = only_clinicvar,
                                                                      use_selected_microbes = use_selected_microbes)
            summarized_results_dict = optimize_random_forest(X_train, mra_train, Y_train,
                                                             X_valid, mra_valid, Y_valid,
                                                             X_tune, mra_tune, Y_tune,
                                                             X_test, mra_test, Y_test)
